...
function Feed() {
  ...
  async function populateTweets() {
    const req = await fetch("http://localhost:5500/feed", {
      headers: {
  ...
  async function addTweets(e) {
    e.preventDefault();
    const req = await fetch(`http://localhost:5500/feed?t=${tweetCount}`, {
      headers: {
        "x-access-token": localStorage.getItem("token"),
      },
    });
    ...
    <div>
      <div className="form-flex">
        <img
          ...
          src={`http://localhost:5500/images/${userAvatar}`}
          // src={`${img}`}
        ></img>
        ...
        <form
          ...
          action="http://localhost:5500/feed"
